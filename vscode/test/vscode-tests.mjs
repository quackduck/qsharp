import * as main_1 from "@vscode/test-web/out/server/main.js";
import * as download_1 from "@vscode/test-web/out/server/download.js";
...
async function getBuild(options) {
  ...
  return await (0, download_1.downloadAndUnzipVSCode)(
  ...
  const browser = await browserType.launch({
    ...
    logger: {
      ...
      log: (name, severity, message, args) =>
        console.log(`playwright: ${name} ${severity} ${message}`),
    },
  });
